...
# нормализация-ключей-объектов
>[!help]
>>[!info] **Интернализация ключей объектов или Нормализация ключей объектов**
>>Когда создаются или запрашиваются свойства объекта, JS преобразует ключи, ***которые могут быть числами, в строковое представление***.
>> Это поведение называется **интернализацией ключей объектов** или **нормализацией ключей** объектов. 
>>Если ключи объекта представляют собой `числа` или `строки`, **которые могут быть преобразованы в одно и то же значение**, они **рассматриваются как одно и то же свойство**.
>>
>>>[!example]
>>>```js
>>>let obj = { "0": 1, 0: 2 };
...
>>
> ###### Пояснение:
>> - Когда создается объект `obj` с ключами `"0": 1` и `0: 2`, ключ `"0"` и ключ `0` рассматриваются как одно и то же свойство.
>>
>> - Значение свойства `"0": 1` сначала записывается, а затем перезаписывается значением `0: 2`.
>>
>> - Итоговый объект `obj` содержит свойство `"0"` со значением `2`, так как последнее значение перезаписало предыдущее.

# мутация-и-создание-новых-объектов
>[!help]
>>[!info] Мутация объекта (изменение оригинального объекта)
>>При мутации вы изменяете существующий объект напрямую. Это экономит память, но может быть нежелательным, если вы хотите сохранить предыдущие данные.
>>>[!example] 
>>>```js
>>>const original = { a: 1, b: 2, c: 3 };
>>>const updates = { b: 42, d: 100 }; // Новые данные
>>>
>>>function mutateObject(obj, updates) {
>>>    for (let key in updates) {
>>>        if (updates.hasOwnProperty(key)) {
>>>            obj[key] = updates[key]; // Изменяем свойства оригинального объекта
>>>        }
>>>    }
>>>}
>>>
>>>mutateObject(original, updates);
>>>
>>>console.log(original);
>>>// Результат: { a: 1, b: 42, c: 3, d: 100 }
>>>
>>>```
>
>>[!info] Создание нового объекта на основе старого
>>Этот подход используется, чтобы ***сохранить оригинальный объект*** неизменным.
>>***Создаётся новый объект***, в который копируются старые данные, а затем добавляются или перезаписываются новые.
>>
>>>[!example] Способ с деструктуризацией (ES6+)
>>>```js
>>>const original = { a: 1, b: 2, c: 3 };
>>>const updates = { b: 42, d: 100 };
>>>
>>>function createNewObject(oldObj, updates) {
>>>    return { ...oldObj, ...updates }; // Копируем свойства из старого и добавляем/перезаписываем новые
>>>}
>>>
>>>const newObject = createNewObject(original, updates);
>>>
>>>console.log(newObject);
>>>// Результат: { a: 1, b: 42, c: 3, d: 100 }
>>>
>>>console.log(original);
>>>// Оригинал остался неизменным: { a: 1, b: 2, c: 3 }
>>>

# передача-копии-ссылки-объекта-в-функцию
>[!help]
>>[!info] 
>>В JS при ***вызове функции*** передаётся ***копия ссылки на объект***, а не сам объект. 
>>Если вы изменяете свойства объекта, то изменения будут видны за пределами функции, потому что объект в памяти остаётся тем же. 
>>Но если внутри функции вы присваиваете переменной новый объект, то это изменяет только ***локальную копию ссылки***, а оригинальная ссылка остаётся неизменной.
>
>>[!example] **Неверный способ: присваивание нового объекта**
>>```js
>>const obj = { a: 1 };
>>
>>function func(obj) {
>>    obj = { a: 2 }; // Локальная копия ссылки теперь указывает на новый объект
>>}
>>
>>func(obj);
>>
>>console.log(obj.a); // 1 (объект остался неизменным)
>>```
>
>>[!example] **Правильный способ: изменение свойств объекта**
>>```js
>>const obj = { a: 1 };
>>
>>function func(obj) {
>>    obj.a = 2; // Изменяем свойство оригинального объекта
>>}
>>
>>func(obj);
>>
...
>[!help] 
>>[!quote]
>> **Event Loop** это механизм, который управляет выполнением кода, обработкой событий и асинхронными задачами..
Сам **Event Loop**  предоставляется средой разработки (***браузер, Node js или что-то другое***)
>
>![[Pasted image 20250115135657.png]]
>
>>[!info] **Основные компоненты Event Loop**
>  1. **Call Stack (Стек вызовов)**  
>       - это структура данных, где хранятся вызовы функций.  
>     ***Пример:*** Если вызвана функция, она добавляется в стек, и выполняется синхронно. После завершения функция удаляется.
>      - ***Роль:*** Управляет выполнением основного синхронного кода сверху вниз.  
>  2. **Web API** 
>       - это механизм, предоставляемый браузером (или Node.js), который выполняет асинхронные операции, такие как:  
>           - `setTimeout`, `setInterval` (таймеры).  
>           - AJAX-запросы, `fetch`.  
>           - Обработчики событий (например, `click`, `keydown`).  
>      - ***Роль:*** 
>           - Асинхронный код отправляется в Web API.  
>           - После выполнения операции (например, отсчёта таймера), Web API передаёт задачу в **очередь макротасок**.  
>  3. **Очередь макротасок (Macrotask Queue)**  
>       - ***Что сюда попадает?***  
>           - Таймеры: `setTimeout`, `setInterval`.  
>           - Коллбеки от Web API (например, события).  
>       - ***Роль:*** Выполняются после завершения всех микротасок. 
>  4. **Очередь микротасок (Microtask Queue)**  
>       - ***Что сюда попадает?***  
>           - `Promise.then` и `catch`.  
>           - `MutationObserver`.  
>           - `queueMicrotask`.  
>       - ***Роль:***  Выполняются **после завершения текущего синхронного кода**, но **перед макротасками**.
>   5. **Heap (Куча)** - это область памяти, где хранятся объекты и данные.  
>       - **Роль:** Предоставляет доступ к памяти для объектов и переменных, которые используются в коде.
>
>>[!info] **Как работает Event Loop?**
>> 1. Синхронный код выполняется в Call Stack.  
>> 2. Асинхронный код отправляется в Web API, где выполняются операции.  
>>3. После завершения операций Web API передаёт задачи:  
>>      - **Микротаски** → Сразу выполняются.  
>>      - **Макротаски** → Выполняются после всех микротасок.  
>>4. Event Loop следит за очередностью выполнения задач.
>
>>[!example] Пример работы
>>```javascript
>>console.log("1"); // Синхронный код → Call Stack. Вывод: 1.
>>
>>setTimeout(() => console.log("2"), 0); // Таймер → Web API, затем очередь макротасок.
>>
>>Promise.resolve().then(() => console.log("3")); // Микротаска → 
>>Очередь микротасок.
>>console.log("4"); // Синхронный код → Call Stack. Вывод: 4.
>>
>>1 4 3 2
>>```
>
>> [!info] **Ситуация с AWAIT**
>![[Pasted image 20250115135818.png]]
>
>> [!info] **Ситуация с цепочками .then().then()**
>![[Pasted image 20250115155226.png]]

# RegExp
>[!help] 
>>[!quote]
>>**Регулярные выражения (RegExp)** - это мощный инструмент для работы с текстом.
>
>###### СОЗДАНИЕ РЕГУЛЯРНОГО ВЫРАЖЕНИЯ 
>>[!example] 2 вариснт создания
>>```js
>>const reg1 = /pattern/;                // литерал
>>const reg2 = new RegExp("pattern");    // конструктор
>>
>>const regExpample = / |,/g // ТУТ говорим что патерн поиска будет ПРОБЕЛ или(|) ЗАПЯТАЯ
>>```
>>
>
>###### ФЛАГИ 
>>[!quote]
>>`/pattern/g` -  **global** - ищет все совпадения
>>`/pattern/i` -  **ignore case** - игнорирует регистр
>>`/pattern/m` - **multiline** - многострочный режим
>>`/pattern/s` - **dotall** - точка включает перенос строки
>>`/pattern/u` -  **unicode** - поддержка Unicode
>>`/pattern/y` -  **sticky** - поиск с точной позиции
>>
>>Таже можно совмещать ФЛАГИ
>>`/pattern/gi` -  **global** и **ignore case**
...
>###### ОСНОВНЫЕ МЕТОДЫ
> ***Для RegExp:***
>`regexp.test(str)`  - возвращает `true/false`
>`regexp.exec(str)`  - возвращает массив с информацией о совпадении
>
> ***Для String:***
>`str.match(regexp)`    // поиск совпадений
>`str.replace(regexp, replacement)` // замена совпадений
>`str.split(regexp)`    // разбиение строки
>`str.search(regexp)`   // поиск позиции
>###### ПРИМЕРЫ
>>[!example] Email
>>```js
...
>>// Разбор шаблона:
>>// ^ - начало строки
>>// [a-zA-Z0-9._-]+ - одна или больше букв, цифр, точек, подчёркиваний или дефисов
>>// @ - символ собаки
>>// [a-zA-Z0-9.-]+ - домен: буквы, цифры, точки или дефисы
>>// \. - точка (экранированная)
>>// [a-zA-Z]{2,6} - от 2 до 6 букв (com, ru, org и т.д.)
>>// $ - конец строки
>>
>>// Примеры использования:
>>emailReg.test("user@gmail.com")      // true
>>emailReg.test("my.email@mail.ru")    // true
>>emailReg.test("invalid@mail")        // false (нет домена верхнего уровня)
>>emailReg.test("no@dots@email.com")   // false (две собаки)
>>```
>
>>[!example] Телефон
>>```js
...
>>
>>// Примеры использования:
>>phoneReg.test("+7(903)123-45-67")    // true
>>phoneReg.test("+7(903)1234567")      // false (нет дефисов)
>>phoneReg.test("89031234567")         // false (другой формат)
>>```
>
>>[!example] Поиск чисел
>>```js
...
>>// \d - любая цифра
>>// + - один или более раз
>>// g - глобальный флаг (искать все совпадения)
>>
>>// Примеры использования:
>>const text = "Цена: 1000 руб., скидка: 100 руб.";
>>text.match(numbersReg)               // ["1000", "100"]
>>
>>const prices = "Товар1: 500, Товар2: 1200, Товар3: 750";
>>prices.match(numbersReg)             // ["1", "500", "2", "1200", "3", "750"]
>>
>>// Можно использовать для извлечения только больших чисел:
>>const bigNumbersReg = /\d{3,}/g;     // только числа от 3 цифр
>>prices.match(bigNumbersReg)          // ["500", "1200", "750"]
>>```
>> [!example] Разные способы поиска разделителей
>>```js
>>const sep1 = / |,|\n/g;        // явное перечисление
>>const sep2 = /\s|,/g;          // пробельные символы или запятая
>>const sep3 = /[,\s]/g;         // через символьный класс
>>```
>###### ПРАКТИЧЕСКИЙ ПРИМЕР
>> [!example] 
>>```js
...
>>
>>// Найти все числа
>>text.match(/\d+/g);            // ["1000", "100"]
>>
>>// Убрать все ПРОБЕЛЫ и ЗАПЯТЫЕ (для Палидрома)
>>const newStr1 = "A man, a plan, a canal, Panama".replace(/ |,/g, '').toLowerCase() // amanaplanacanalpanama
>>
>>// Разбить строку по пробелам и знакам препинания
>>text.split(/[,\s]/g);          // ["Цена:", "1000", "руб.", "", "скидка:", "100", "руб."]
>>```
>
